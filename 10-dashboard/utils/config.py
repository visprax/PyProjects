...
class Config:
    ...
    def get_config(self):
        ...
        """
        logger.info(f"reading config file: {self.filepath}")
            ...
            if section in config.sections():
                for key in params[section].keys():
                    try:
                        ...
                        self.log_missing(section, key, params[section][key])
            else:
        ...
        if "finnhub" in config.sections():
            try:
                ...
                self.log_missing("finnhub", "key")

            try:
                if config["finnhub"]["frequency"]:
                    frequency = config["finnhub"]["frequency"]
                    frequency = self.strip_quotes(frequency)
                    finnhub_defaults["frequency"] = frequency
            except KeyError:
                self.log_missing("finnhub", "frequency", finnhub_defaults["frequency"])

            try:
                if config["finnhub"]["symbols"]:
                    symbols = config["finnhub"]["symbols"]
                    symbols = self.strip_quotes(symbols)
                    symbols = symbols.replace(' ', '').split(',')
                    finnhub_defaults["symbols"] = symbols
            except KeyError:
                self.log_missing("finnhub", "symbols", finnhub_config["symbols"])
        else:
            logger.warn(f"{section} not in config file, setting to default values")
    ...
    def log_missing(self, section, key, default=None):
